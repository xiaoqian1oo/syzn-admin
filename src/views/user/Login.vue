      ...
      <a-form-item>
        <a-input
          ...
          v-decorator="[
            'account',
            {
              rules: [{ required: true, message: '请输入登录账户' }, { validator: handleUsernameOrEmail }],
...
import { getSmsCaptcha,login } from '@/api/login'
...
export default {
  ...
  methods: {
    ...
    loginSuccess(res) {
      storage.set('username',res.user.username)
      storage.set('account',res.user.account)
